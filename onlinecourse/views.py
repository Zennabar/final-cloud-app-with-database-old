...
def submit(request, course_id):
    user = request.user
    course = get.get_object_or_404(course_id)
...
def show_exam_result(request, course_id, submission_id):
    ...
    selected_choice = Submission.objects.get(id = submission_id).values_list('choices')
    points = 0
    questions = 0

    for answer in submit.choices.all():
        if(answer.is_correct):
            points += 1
        questions += 1
    ctx['selected_choice'] = selected_choice
    ctx['grade'] = int(points/questions * 100)
    ctx['course'] = course
